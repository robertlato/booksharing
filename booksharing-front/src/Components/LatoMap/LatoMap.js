...
class LatoMap extends React.Component {
  ...
  componentDidMount() {
    ...
    this.popup.setElement(ReactDOM.findDOMNode(this).querySelector('#popup'));
    ...
    this.map.on('click', e => {
      ...
      if (this.map.hasFeatureAtPixel(pixel)) {
        ...
        let isBorrowed = myFeatures[0].get("borrowed") ? "wypożyczona" : "dostępna";
        let query = 
  ...
  fetchCoords = async (city, street, houseNumber) => {
    try {
      const url = "https://nominatim.openstreetmap.org/search?city=" + city + "&street=" + houseNumber + "%20" + street + "&format=json";
